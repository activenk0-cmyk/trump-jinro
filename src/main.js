...
const SHEET_W = 1120;
const SHEET_H = 928;
// 各列・各行の「カード左上」ピクセル座標（実測）。ズレたらこの値だけ調整。
const SPRITE_COL_X = [4, 190, 376, 562, 748, 934];   // 6列
const SPRITE_ROW_Y = [4, 190, 376, 562, 748];        // 5行
const CELL_W = 182; // 1枚の描画対象領域（枠内）幅
const CELL_H = 180; // 1枚の描画対象領域（枠内）高さ
...
// displayW（カードの表示幅px）に合わせ、背景サイズと位置をpxで返す。
function spriteStyle(index, displayW) {
  ...
  const posX = -(SPRITE_COL_X[col]) * scale;
  const posY = -(SPRITE_ROW_Y[row]) * scale;
...
function shuffleArr(array) { for (let i = array.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [array[i], array[j]] = [array[j], array[i]]; } return array; }
...
function endGame(state, winnerSlot, reason) { state.status = "ended"; state.winner = winnerSlot; state.reason = reason; addLog(state, "🎉 ゲーム終了：プレイヤー" + winnerSlot + "の勝利（" + reason + "）"); }
...
function checkConstraintForCitizenPlay(state, slot, card) {
  ...
  if (constraint.type === "forceAccuse") throw new GameActionError("前のターンの効果により、今回は告発しか行えません。");
  if (constraint.type === "forceAttribute") {
    const qualifying = state.hands[slot].filter((c) => c.type === "citizen" && (constraint.attr === "mark" ? c.mark === constraint.value : c.number === constraint.value));
    if (qualifying.length > 0) {
      const matches = constraint.attr === "mark" ? card.mark === constraint.value : card.number === constraint.value;
      if (!matches) throw new GameActionError(`前のターンの効果により、「${constraint.value}」の付いた市民カードを出す必要があります。`);
    }
  }
}
function reducePlayCitizen(state, slot, cardId) {
  ...
  if (state.status === "playing") { setLastAction(state, slot, card.mark + card.number); finishTurn(state, slot); }
  return state;
}
function computeSeerResult(state, slot) {
  ...
  if (slot === "A") {
    ...
    if (candidates.length === 0) throw new GameActionError("これ以上、開示できる安全な数字がありません。");
    ...
    if (candidates.length === 0) throw new GameActionError("これ以上、開示できる安全なマークがありません。");
    const v = candidates[Math.floor(Math.random() * candidates.length)];
...
function reducePlayRole(state, slot, cardId, options = {}) {
  ...
  if (constraint) {
    if (constraint.type === "forceAccuse") throw new GameActionError("前のターンの効果により、今回は告発しか行えません。");
    if (constraint.type === "forceAttribute") {
      const qualifying = hand.filter((c) => c.type === "citizen" && (constraint.attr === "mark" ? c.mark === constraint.value : c.number === constraint.value));
      if (qualifying.length > 0) throw new GameActionError(`前のターンの効果により、市民カード（「${constraint.value}」の付いたもの）を出す必要があります。`);
    }
    if (constraint.type === "blockRoles") {
      const hasCitizen = countCitizens(hand) > 0;
      if (hasCitizen) throw new GameActionError("前のターンの効果により、役職カードは出せません。市民カードを出してください。");
  ...
  const hasEnoughWithVirtual = !hasEnoughByPool && meta.virtualCostUnused && availableCost + 1 >= requiredCost;
  ...
  if (card.type === "role" && card.role === "J") pendingSeer = computeSeerResult(state, slot);
  if (card.type === "role" && card.role === "Q") {
    if (!options.attr || (options.attr !== "mark" && options.attr !== "number") || !options.value) throw new GameActionError("怪盗の効果には、指定するマークまたは数字が必要です。");
  }
  if (hasEnoughByPool) state.costPool[slot] = availableCost - requiredCost;
  else { meta.virtualCostUnused = false; const remaining = requiredCost - 1; state.costPool[slot] = Math.max(0, availableCost - remaining); }
  hand.splice(idx, 1);
...
function reduceAccuse(state, slot, mark, number) {
  ...
  if (constraint && constraint.type !== "forceAccuse") {
    if (constraint.type === "forceAttribute") {
      const qualifying = state.hands[slot].filter((c) => c.type === "citizen" && (constraint.attr === "mark" ? c.mark === constraint.value : c.number === constraint.value));
      if (qualifying.length > 0) throw new GameActionError("前のターンの効果により、市民カードを出す必要があるため告発できません。");
    }
    if (constraint.type === "blockRoles") {
      const hasCitizen = state.hands[slot].some((c) => c.type === "citizen");
      if (hasCitizen) throw new GameActionError("前のターンの効果により、市民カードを出す必要があるため告発できません。");
    }
  }
...
// Firestore
// ============================================================
function normalizeRoomId(roomId) { return String(roomId || "").trim().toUpperCase().slice(0, 6); }
...
const Game = {
  ...
  joinExisting: async (idInput) => { const id = normalizeRoomId(idInput); const snap = await getDoc(roomRef(id)); if (!snap.exists()) throw new GameActionError("そのルームIDが見つかりません。IDを確認してください。"); const state = await runAction(id, (s) => reduceJoin(s, "B")); return { id, state }; },
  updateSettings: (roomId, slot, infoType, winCount, jokerCost) => runAction(roomId, (s) => reduceUpdateSettings(s, slot, infoType, winCount, jokerCost)),
...
const CARD_W = { field: 46, hand: 88 };

function cardVisualHtml(card, opts = {}) {
  const { interactive = false, disabled = true, dataCardId = null, extraClass = "", displayW = 46 } = opts;
  ...
  const frameClass = card.type === "citizen" ? (RED_MARKS.includes(card.mark) ? "suit-red" : "suit-black") : card.type === "role" ? "role-frame" : "joker-frame";
  return `<button ${disabledAttr} ${dataAttr} class="card ${frameClass} ${interactiveClass} ${extraClass}" style="background-image:url('/cards.png'); ${styleSprite}"></button>`;
}
function handCardHtml(card, canAct) {
  const kindClass = card.type === "citizen" ? "citizen-hand" : "role-hand";
  return cardVisualHtml(card, { interactive: canAct, disabled: !canAct, dataCardId: card.id, extraClass: "hand-card " + kindClass, displayW: CARD_W.hand });
}
...
function showModal(html) { const overlay = document.getElementById("modalOverlay"); document.getElementById("modalContent").innerHTML = html; overlay.classList.add("show"); }
function closeModal() { const overlay = document.getElementById("modalOverlay"); if (overlay) overlay.classList.remove("show"); }
function showSimpleModal(text) { showModal(`<div class="big-text">${text}</div><button onclick="window.__closeModal()">閉じる</button>`); }
function showError(err) { const msg = err instanceof Error ? err.message : String(err); showSimpleModal("⚠️ " + msg); }
function openSheet() { document.getElementById("sheetOverlay").classList.add("show"); }
function closeSheet() { const overlay = document.getElementById("sheetOverlay"); if (overlay) overlay.classList.remove("show"); }
...
async function handleCreateRoom() {
  try { const { id } = await Game.createRoom(); roomId = id; mySlot = "A"; saveSession(); lastSeenSeq = 0; endAnnounced = false; startWatching(); }
...
async function handleJoinRoomSubmit() {
  ...
  try { const { id, state } = await Game.joinExisting(val); roomId = id; mySlot = "B"; saveSession(); lastSeenSeq = (state.lastAction && state.lastAction.seq) || 0; endAnnounced = state.status === "ended"; startWatching(); }
  catch (err) { showError(err); }
}
...
function leaveRoom() {
  if (unsubscribe) unsubscribe();
  unsubscribe = null; roomId = null; mySlot = null; clearSession();
  const eo = document.getElementById("endOverlay"); if (eo) eo.remove();
  renderLandingScreen();
}
function confirmLeaveMidGame() {
  showModal(`<div class="big-text">退室しますか？</div><div style="font-size:12px;color:#f4b400;margin-bottom:10px;">対戦中でも自分の画面から抜けられます。</div><button class="danger" onclick="window.__leaveConfirmed()">退室する</button><button class="secondary" onclick="window.__closeModal()">キャンセル</button>`);
}
...
function renderWaiting(state) {
  ...
  document.getElementById("copyIdBtn").onclick = () => { navigator.clipboard.writeText(roomId).then(() => { const btn = document.getElementById("copyIdBtn"); btn.textContent = "✅ コピーしました"; setTimeout(() => (btn.textContent = "📋 コピー"), 1500); }); };
  document.getElementById("saveSettingsBtn").onclick = async () => {
    const infoType = document.getElementById("infoTypeSelect").value, winCount = document.getElementById("winCountSelect").value, jokerCost = document.getElementById("jokerCostSelect").value;
    try { await Game.updateSettings(roomId, mySlot, infoType, winCount, jokerCost); showSimpleModal("設定を保存しました。"); } catch (err) { showError(err); }
  };
  document.getElementById("startBtn").onclick = async () => { try { lastSeenSeq = 0; endAnnounced = false; await Game.start(roomId); } catch (err) { showError(err); } };
...
function renderPlaying(state) {
  ...
  const citizenGridHtml = MARKS.map((mark) => NUMBERS.map((num) => statusCellHtml(mark, num, playedSet.has(mark + num))).join("")).join("");
  ...
  let turnChipHtml;
  if (isEnded) { const won = state.winner === mySlot; turnChipHtml = `<div class="chip turn-chip ${won ? "win-chip" : "lose-chip"}">${won ? "🏆 VICTORY" : "💀 DEFEAT"}</div>`; }
  else { turnChipHtml = `<div class="chip turn-chip ${isMyTurn ? "my-turn" : "opp-turn"}">${isMyTurn ? "🎯 あなたのターン" : "⌛ 相手のターン"}</div>`; }

  const handHtml = (state.hands?.[mySlot] || []).map((card) => handCardHtml(card, isMyTurn && state.phase === "needAction")).join("");
  ...
  if (mulliganBtn) mulliganBtn.onclick = () => { showModal(`<div style="color:#f4b400;font-size:13px;margin-bottom:10px;">⚠️ 手札を相手に公開してから引き直します。よろしいですか？</div><button class="warning-btn" onclick="window.__mulligan()">引き直す</button><button class="secondary" onclick="window.__closeModal()">キャンセル</button>`); };
  ...
  if (logBtn) logBtn.onclick = () => { showModal(`<div class="flat-label" style="margin-bottom:8px;">📝 ゲームログ</div><div class="log-box" style="text-align:left;">${(state.log || []).map((l) => `<div>${l}</div>`).join("")}</div><button onclick="window.__closeModal()" style="margin-top:12px;">閉じる</button>`); };
  ...
  if (submitAccuse) submitAccuse.onclick = () => {
    const mark = document.getElementById("accuseMark").value, num = document.getElementById("accuseNumber").value;
    closeSheet();
    showModal(`<div>人狼は「${mark}の${num}」だと告発しますか？<br><span style="color:#f4b400;font-size:13px;">外すと即敗北です</span></div><button class="danger" onclick="window.__accuse('${mark}', ${num})">告発する</button><button class="secondary" onclick="window.__closeModal()">キャンセル</button>`);
  };

  const rematchBtn = document.getElementById("rematchBtn");
  if (rematchBtn) rematchBtn.onclick = () => { lastSeenSeq = 0; endAnnounced = false; const eo = document.getElementById("endOverlay"); if (eo) eo.remove(); Game.reset(roomId).catch(showError); };
...
function confirmPlayRole(card) {
  if (card.type === "role" && card.role === "Q") {
    ...
    const attrSel = document.getElementById("qAttrSelect"), valSel = document.getElementById("qValueSelect");
    const refresh = () => { valSel.innerHTML = attrSel.value === "mark" ? MARKS.map((m) => `<option value="${m}">${m}</option>`).join("") : NUMBERS.map((n) => `<option value="${n}">${n}</option>`).join(""); };
    attrSel.onchange = refresh; refresh();
    return;
  }
...
window.__playRole = (cardId, options) => {
  ...
  Game.playRole(roomId, mySlot, cardId, options).catch((err) => {
    if (err.payload?.emergencyAvailable) showModal(`<div style="color:#f4b400;font-size:13px;margin-bottom:10px;">⚠️ コストが足りません。手札に市民カードがなく、コストも0のため、緊急セット（裏向きで出す）が可能です。</div><button class="warning-btn" onclick="window.__playRole('${cardId}', {emergencySet:true})">緊急セットする</button><button class="secondary" onclick="window.__closeModal()">キャンセル</button>`);
    else if (err.payload?.kForcedFacedownAvailable) showModal(`<div style="color:#f4b400;font-size:13px;margin-bottom:10px;">⚠️ 騎士の効果により、役職カードは効果を発動できません。手札に市民カードが無いため、裏向きで場に出されます。</div><button class="warning-btn" onclick="window.__playRole('${cardId}', {forceFacedownByBlock:true})">裏向きで出す</button><button class="secondary" onclick="window.__closeModal()">キャンセル</button>`);
    else showError(err);
  });
};
window.__playRoleQ = (cardId) => { const attr = document.getElementById("qAttrSelect").value, value = document.getElementById("qValueSelect").value; window.__playRole(cardId, { attr, value: attr === "number" ? Number(value) : value }); };
...
function renderCreditBadge() {
  ...
  const el = document.createElement("div");
  el.id = "grokCredit"; el.className = "grok-credit"; el.textContent = "✨ Created with Grok";
  document.body.appendChild(el);
}
renderCreditBadge();
